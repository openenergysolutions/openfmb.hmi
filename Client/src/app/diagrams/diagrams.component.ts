...
export class DiagramsComponent implements OnInit, OnDestroy {
  ...
  delete(row: any) {
    if (confirm("Are you sure to delete diagram name " + row.name)) {
      this.service.delete(row.diagramId).subscribe(
        (_data) => {
  ...
  addOrEdit(data: any = {}, isNew?) {
    const title = isNew ? "Add new diagram" : "Update diagram";
    ...
    dialogRef.afterClosed().subscribe((res) => {
      ...
      const diagram: Diagram = {
        ...
        backgroundColor:
          res.backgroundColor && res.backgroundColor.hex
            ? "#" + res.backgroundColor.hex
            : "",
      ...
      try {
        if (isNew) {
          this.service.create(diagram).subscribe(
            (_data) => {
          ...
          this.service.update(diagram).subscribe(
            (_data) => {
