...
import { Helpers, Symbol } from "../shared/hmi.constants";
...
export class DesignerComponent implements AfterViewInit, OnDestroy {
  ...
  sessionId = "";
  defaultLabelStyle =
    "text;html=1;strokeColor=none;fontColor=#000000;verticalAlign=middle;whiteSpace=wrap;rounded=0;fillColor=none;";
  defaultButtonStyle =
    "html=1;strokeColor=none;verticalAlign=middle;whiteSpace=wrap;rounded=1;";
  defaultSetpointButtonStyle =
    "html=1;strokeColor=none;align=center;verticalAlign=middle;whiteSpace=wrap;rounded=1;fillColor=#a6a6af;fontColor=#0e0e0f;";
  defaultStatusIndicatorStyle =
    "html=1;strokeColor=none;align=left;verticalAlign=middle;rounded=1;fillColor=none;fontColor=#000000;";
  defaultRectangleGroupStyle =
    "rounded=0;whiteSpace=wrap;html=1;dashed=1;shadow=0;sketch=0;glass=0;fillColor=none;strokeWidth=2;strokeColor=#606060";
  gridData = {
    scale: 0,
    gridSize: 0,
  ...
  ngAfterViewInit() {
    ...
    this.store
      .select((state) => state.designer.connectColor)
      .pipe(takeUntil(this.destroy$))
      .subscribe((data) => {
        if (data) {
  ...
  private graphInit() {
    ...
    this.graph.getModel().addListener(mxEvent.UNDO, (_sender, evt) => {
      this.undoManager?.undoableEditHappened(evt.getProperty("edit"));
    });
    this.graph.getView().addListener(mxEvent.UNDO, (_sender, evt) => {
      this.undoManager?.undoableEditHappened(evt.getProperty("edit"));
    });
    ...
    this.graph.insertEdge = (...params) => {
      const color = this.graph
        .getStylesheet()
        .getDefaultEdgeStyle().strokeColor;
      ...
      const cell = insertEdge.apply(this.graph, params);
    ...
    this.graph.getAllConnectionConstraints = (terminal, _source) => {
      if (
        terminal != null &&
        terminal.shape != null &&
    ...
    // Handle undo (ctrl-z)
    keyHandler.bindControlKey(90, (_evt: Event) => {
      if (this.graph.isEnabled()) {
    ...
    // Handle redo (ctrl-y)
    keyHandler.bindControlKey(89, (_evt: Event) => {
      if (this.graph.isEnabled()) {
  ...
  private addSidebarIcon(
    graphItem: mxgraph.mxGraph,
    sidebar: ElementRef,
    data: any,
  ) {
    const imgDropFunction = (
      graph: mxgraph.mxGraph,
      evt: Event,
      cell: mxgraph.mxCell,
      x: number,
        ...
        try {
          if (data.shape == Symbol.image) {
            ...
            v1 = graph.insertVertex(
              parent,
              this.idGenerator(),
              data.label === false ? null : userObject,
              pointX,
              pointY,
              data.width ? data.width : 40,
              data.height ? data.height : 40,
              `shape=${data.shape};${
            ...
            data.points.forEach((point) => {
              ports.push({
                ...
                y: point[1],
                perimeter:
                  data.pointPerimeter === false ? data.pointPerimeter : true,
              });
            });
            ...
            v1 = graph.insertVertex(
              ...
              this.defaultButtonStyle +
                "align=center;" +
                "fillColor=" +
                Helpers.buttonBackColor() +
                ";fontColor=" +
                Helpers.buttonForeColor() +
                ";",
            );
            ...
            v1 = graph.insertVertex(
              ...
              80,
              40,
              this.defaultSetpointButtonStyle,
            );
            ...
            v1 = graph.insertVertex(
              ...
              80,
              40,
              this.defaultStatusIndicatorStyle,
            );
            ...
            v1 = graph.insertVertex(
              ...
              80,
              40,
              this.defaultRectangleGroupStyle,
            );
            ...
            const cell = new mxCell(
              "",
              new mxGeometry(0, 0, 80, 80),
              "endArrow=none;html=1;edgeStyle=isometricEdgeStyle;strokeColor=#000000;",
            );
            ...
            const cell = new mxCell(
              ...
              "endArrow=none;html=1;curved=1;strokeColor=#000000;",
            );
            cell.id = this.idGenerator();
    ...
    this.renderer.setAttribute(
      img,
      "src",
      data.image.split(".svg")[0] + "-copy.svg",
    );
    ...
    this.renderer.setStyle(
      dragSource,
      "width",
      (data.width ? data.width : 40) + "px",
    );
    this.renderer.setStyle(
      dragSource,
      "height",
      (data.height ? data.height : 40) + "px",
    );
  ...
  private drawGrid() {
    try {
      ...
      mxGraphView.prototype.isContainerEvent = (evt: Event) => {
        return (
          mxGraphViewIsContainerEvent.apply(this.graph.getView(), [evt]) ||
          mxEvent.getSource(evt) === this.canvas
        );
      };
      this.repaintGrid(
        new ElementRef(this.canvas),
        this.graph,
        this.gridData.scale,
        this.gridData.gridSize,
        this.gridData.translationPoint,
        this.gridData.width,
  ...
  openDialog(x: number, y: number, cell: mxgraph.mxCell): void {
    ...
    dialogRef
      ...
      .subscribe((result) => {
        if (result) {
          console.log(
            "Property dialog closed.  Saving data..." + JSON.stringify(result),
          );
          ...
          if (
            ...
            if (userObject.foreColor) {
              const style =
                "endArrow=none;html=1;edgeStyle=isometricEdgeStyle;strokeColor=" +
                userObject.foreColor;
              this.graph.setCellStyle(style, [cell]);
            } else {
              const style =
                "endArrow=none;html=1;edgeStyle=isometricEdgeStyle;strokeColor=#000000";
            ...
            if (userObject.foreColor) {
              const style =
                "endArrow=none;html=1;curved=1;strokeColor=" +
                userObject.foreColor;
              this.graph.setCellStyle(style, [cell]);
            } else {
              const style =
          ...
          if (result.navigateToDataConnection === true) {
            ...
            this.naviator.navigateByUrl(
              "/data-connect?id=" + this.diagramId + "&cell=" + cell.id,
            );
          }
        }
      });
  ...
  loadGraphFromFile(file: File) {
    if (file) {
      ...
      fileReader.onload = () => {
        const xml = mxUtils.parseXml(fileReader.result);
