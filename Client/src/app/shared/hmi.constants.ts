...
export const Hmi = {
  ...
  isBattery: (type: string) => {
    if (
      type === Symbol.battery ||
      type === Symbol.batteryHorizontal ||
      type === Symbol.battery3D
    ) {
      return true;
    }
    return false;
  ...
  isPowerFlow: (type: string) => {
    return (
      type && (type.startsWith(Symbol.arrow) || type.startsWith(Symbol.flow))
    );
  },
  isLabel: (type: string) => {
  ...
  isVisibilitySupport: (type: string) => {
    return (
      Hmi.isPowerFlow(type) ||
      Hmi.isMeasureBox(type) ||
      Hmi.isLabel(type) ||
      Hmi.isStatusIndicator(type)
    );
  },
};
...
export const Helpers = {
  ...
  getWeatherStatusIcon: (
    val: number,
    definitions: WeatherStatusDefinition[],
  ) => {
    let ret = "";
