...
async fn server_setup() {
    ...
    let _ = write_hmi_env(&hmi_local_ip, &http_scheme, &ws_scheme, &auth_audience, &auth_client_id, &auth_domain, &auth_scope, &auth_authorize_path);
...
fn write_hmi_env(hmi_local_ip: &str, http_scheme: &str, ws_scheme: &str, auth_audience: &str, auth_client_id: &str, auth_domain: &str, auth_scope: &str, auth_authorize_path: &str) -> std::io::Result<()> {
